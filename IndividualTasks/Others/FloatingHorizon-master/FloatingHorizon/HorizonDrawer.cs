...
namespace FloatingHorizon
{
    ...
    enum VisibilityType //типи видимості
    {
        VisibleAndUpper,
        VisibleAndLower,
        Invisible
    ...
    class HorizonDrawer
    {
        ...
        public void Draw(Graphics imageGraphics, functionType function)
        {
            ...
            StartDoubleBuffering();//очистка екрану і згладжування
            ResetHorizons();
        ...
        private bool CheckPoint(Point2D point) //не вилазить за межі
        {
        ...
        private void DrawHelperX(Graphics imageGraphics, functionType function) //малювання по х точки
        {
            Point2D prevPoint,
            ...
            for (double currentX = xMax; currentX >= xMin; currentX -= xStep)
            {
                ...
                ProcessEdge(prevPoint, ref leftPoint); //малює лінії коли за межами границь
                if (!(CheckPoint(prevPoint) && CheckPoint(leftPoint)))
                    continue;

                VisibilityType prevVisibility = CheckVisibility(prevPoint); //аналіз точки над чи під горизонтом
                ...
                for (double currentZ = zMin; currentZ <= zMax; currentZ += zStep)
                {
                    ...
                    {
                        ProcessHorizonIntersectingLine(currentVisibility, prevVisibility, prevPoint, currentPoint); //малює лінію між точками над і під горизонтом
                    }
        ...
        private void StartDoubleBuffering() //очистка екрану і згладжування
        {
        ...
        private void ProcessHorizonIntersectingLine(VisibilityType currentVisibility, VisibilityType prevVisibility,
           Point2D prevPoint, Point2D currentPoint) //малює лінію між точками над і під горизонтом
        {
            Point2D currentIntersection; //перехрестя
        ...
        private Point2D ProcessVisibleNowLine(VisibilityType prevVisibility, Point2D currentPoint, Point2D prevPoint, bool Flag)
        {
            ...
            if (prevVisibility == VisibilityType.Invisible) // стан попередньої точки - невидимий
            {
                currentIntersection = GetCurvesIntersection(prevPoint, currentPoint, upperHorizon);
        ...
        private VisibilityType CheckVisibility(Point2D currentPoint) //перевіряє видимість точки у над чи під горизонтами
        {
        ...
        private void ProcessEdge(Point2D prevPoint, ref Point2D currentPoint)//малює лінію якщо за межами горизонтів
        {
        ...
        private void LineApproximate(Point2D firstPoint, Point2D secondPoint)//зміна горизонта
        {
        ...
        private void Smooth(Point2D firstPoint, Point2D secondPoint) //визначення горизонтів
        {
